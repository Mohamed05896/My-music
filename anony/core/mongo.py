...
class MongoDB:
    ...
    async def migrate_coll(self) -> None:
        ...
        try:
            musers, mchats = [], []
            user_ids_seen, chat_ids_seen = set(), set()
            ...
            for user in ulist:
                if isinstance(user.get("_id"), ObjectId):
                    ...
                    if not user_id or user_id in user_ids_seen:
                        continue
            ...
            async for chat in self.chatsdb.find():
                if isinstance(chat.get("_id"), ObjectId):
                    ...
                    if not chat_id or chat_id in chat_ids_seen:
                        continue
            ...
            await self.cache.update_one({"_id": "migrated"}, {"$set": {"done": True}}, upsert=True)
            logger.info("Migration completed.")
        except Exception as e:
            logger.warning(f"Migration encountered issues: {type(e).__name__} - {e}")
            await self.cache.update_one({"_id": "migrated"}, {"$set": {"done": True}}, upsert=True)
            logger.info("Migration marked as complete despite errors.")
